...
public static class AdminSecurityServiceCollectionExtensions
{
	...
	{
		builder.AddPolicy(AdminPortalPolicies.SearchAggregates,
			policy => policy.AddRequirements(
				new AdminFeatureRequirement(AdminFeature.SearchAggregates)));

		builder.AddPolicy(AdminPortalPolicies.ViewAggregate,
			policy => policy.AddRequirements(
				new AdminFeatureRequirement(AdminFeature.ViewAggregate),
				new AggregateTypeAccessRequirement()));

		builder.AddPolicy(AdminPortalPolicies.ViewEvents,
			policy => policy.AddRequirements(
				new AdminFeatureRequirement(AdminFeature.ViewEvents),
				new AggregateTypeAccessRequirement()));

		builder.AddPolicy(AdminPortalPolicies.ProjectPointInTime,
			policy => policy.AddRequirements(
				new AdminFeatureRequirement(AdminFeature.ProjectPointInTime),
				new AggregateTypeAccessRequirement()));

		builder.AddPolicy(AdminPortalPolicies.ExportEvents,
			policy => policy.AddRequirements(
