...
static class CosmosDbClientExtensions
{
	...
	public static async IAsyncEnumerable<T> GetQueryEnumerableAsync<T>(
		...
		where T : class
	{
		...
		do
		{
			var response = await cosmosDbClient.QueryAsync(
				whereClause,
				orderByClause,
				request,
				partitionKey,
	...
	public static IAsyncEnumerable<T> GetQueryEnumerableAsync<T>(
		...
		where T : class =>
		cosmosDbClient.GetQueryEnumerableAsync(
			whereClause,
			null,
			partitionKey,
	...
	public static async IAsyncEnumerable<T> GetListEnumerableAsync<T>(
		[NotNull] this CosmosDbClient cosmosDbClient,
		Func<IQueryable<T>, IQueryable<T>>? orderByClause,
		PartitionKey partitionKey,
		int maxRecordsPerOperation = ContinuationRequest.DefaultMaxRecords,
	...
	public static Task<ContinuationResponse<T>> QueryAsync<T>(
		...
		where T : class =>
		cosmosDbClient.QueryAsync(
			whereClause,
			orderByClause,
			new ContinuationRequest { MaxRecords = maxRecords, ContinuationToken = null },
			partitionKey,
			cancellationToken
		);
	...
	public static async Task<IEnumerable<ResponseMessage>> DeleteAsync<T>(
		[NotNull] this CosmosDbClient cosmosDbClient,
