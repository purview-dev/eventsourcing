...
static class EmitHelper
{
	...
	)
	{
		...
		foreach (var method in info.Methods)
		{
			GenerateCommandMethod(writer, info, method, indent);
		}
	...
	)
	{
		...
		{
			writer.WriteLine(
				$"\t\tpublic {prop.PropertyTypeName} {prop.PropertyName} {{ get; set; }} = default!;"
			);
		}
	...
	)
	{
		if (method.IsCollectionEvent)
		{
			GenerateCollectionCommandMethod(writer, method, indent);
			return;
		}
		...
		static void GenerateCollectionCommandMethod(
			CodeWriter writer,
			AggregateEventMethodInfo method,
			string indent
		)
		...
		EmitEventCreation(writer, method, storedParameters, indent, declareVariable: true);
		writer.WriteLine($"{indent}\t\tif (!ShouldApply{hookSuffix}(@event))");
		writer.WriteLine($"{indent}\t\t{{");
		EmitNoChangeReturn(writer, method.ReturnKind, indent, 3);
		writer.WriteLine($"{indent}\t\t}}");
		...
		writer.WriteLine();
		EmitEventCreation(writer, method, storedParameters, indent, declareVariable: false);
		writer.WriteLine($"{indent}\t\tif (!ShouldApply{hookSuffix}(@event))");
		writer.WriteLine($"{indent}\t\t{{");
		EmitNoChangeReturn(writer, method.ReturnKind, indent, 3);
		writer.WriteLine($"{indent}\t\t}}");
	...
	)
	{
		...
		if (storedParameters.Count > 0)
		{
			...
			{
				writer.WriteLine(
					$"{indent}\t\t\t{prop.PropertyName} = {GetWorkingValueName(prop)},"
				);
			}
	...
	static string GetWorkingValueName(EventPropertyInfo parameter) =>
		...
		|| parameter.ParameterConversionKind is not EventParameterConversionKind.None
