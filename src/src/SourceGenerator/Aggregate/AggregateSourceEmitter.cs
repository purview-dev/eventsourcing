...
static partial class AggregateSourceEmitter
{
	public static void GenerateAggregateSource(AggregateGenerationOutputContext outputContext)
	{
		...
		outputContext.Writer.WriteClass(
			new(outputContext.Aggregate.AggregateClass.TypeValue.TypeName)
			{
				Accessibility = GetAccessModifier(outputContext.Aggregate.Accessibility),
				IsPartial = true,
				IsSealed = false,
	...
	)
	{
		...
		outputContext.Writer.WriteClass(
			new(method.EventType.TypeValue.TypeName)
			{
	...
	static void GenerateJsonSerializationSupport(AggregateGenerationOutputContext outputContext)
	{
		...
		outputContext.Writer.WriteMethod(
			...
			{
				writeBody
					.Write("var aggregate = new ")
					.Write(outputContext.Aggregate.AggregateClass);
				using (writeBody.OpenBlockScope())
				{
					...
					foreach (var property in outputContext.Aggregate.Properties)
						writeBody.WriteLine(
							$"{property.PropertyName} = jsonModel.{property.PropertyName},"
						);
				}
		...
		outputContext.Writer.WriteMethod(
			...
			applyBody =>
			{
				...
				foreach (var prop in mappedParameters)
					applyBody.WriteAssignment(
						prop.AggregatePropertyName,
						$"{eventParameterName}.{prop.PropertyName}"
					);
				...
				applyBody.WriteMethodCall(
					$"OnApplied{GetHookName(method.EventType)}",
					eventParameterName
				);
			}
		);
		...
		foreach (var prop in aggregateProperties)
		{
			writer.WritePartialMethod(
				new($"On{prop.AggregatePropertyName}Changing")
				{
					...
					Parameters =
					[
						new(prop.ParameterName, prop.PropertyType)
						{
							Modifier = ParameterModifier.Ref,
						},
					],
	...
	)
	{
		var writer = outputContext.Writer;
		writer
			.Write(declareVariable ? "var @event = new " : "@event = new ")
			.Write(method.EventType);
	...
	)
	{
		...
		using (outputContext.Writer.OpenBlockScope(method.Signature))
		{
			outputContext.Writer.WriteThrow(
				$"new global::System.InvalidOperationException(\"{message}\")"
			);
		}
			...
			bodyWriter =>
			{
				bodyWriter.WriteMethod(
					...
					)
					{
						...
						Parameters =
						[
							new("reader", TypeLibrary.System.TextJson.Utf8JsonReader)
							{
								Modifier = ParameterModifier.Ref,
							},
	...
	static void GenerateJsonModel(AggregateGenerationOutputContext outputContext)
	{
		...
		writer.WriteClass(
			new($"{outputContext.Aggregate.AggregateClass.TypeName}JsonModel")
			{
				Accessibility = TypeDeclarationAccessibility.Internal,
				IsPartial = false,
				IsSealed = true,
