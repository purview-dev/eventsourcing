...
static class TypeHelperExtensions
{
	...
	extension(TypeHelpers)
	{
		...
		)
		{
			...
			var declaration = methodSymbol
				...
				.FirstOrDefault(static syntax =>
					syntax.Modifiers.Any(static modifier =>
		...
		public static bool TryCreateEventMethodInfo(
			INamedTypeSymbol classSymbol,
			IMethodSymbol methodSymbol,
		...
		{
			var eventSuffix = (
				aggregateEventSuffixOverride ?? assemblyEventSuffix ?? "Event"
			).Trim();
			...
			foreach (var parameter in methodSymbol.Parameters)
			{
				...
				if (
					parameter.IsParams
					&& (!isCollectionEvent || parameter.Type is not IArrayTypeSymbol)
				)
			...
			if (isCollectionEvent)
			{
				if (methodSymbol.Parameters.Length != 1)
					ReportUnsupportedSignature(
						"collection event methods must have exactly one parameter"
					);
			}
			...
			if (activeAttribute is not null)
			{
				...
				{
					if (
						namedArgument.Key == "Version"
						&& namedArgument.Value.Value is int explicitVersion
					)
					{
					...
					if (
						namedArgument.Key == "EventName"
						&& namedArgument.Value.Value is string explicitEventName
					)
					{
					...
					if (
						namedArgument.Key == "EventNamespace"
						&& namedArgument.Value.Value is string explicitEventNamespace
					)
					{
					...
					if (
						namedArgument.Key == "Manual"
						&& namedArgument.Value.Value is bool explicitManual
					)
		...
		)
		{
			...
			var hasScalarAttribute = TypeHelpers.HasAttribute(
				propertyType,
				TypeLibrary.Attributes.ScalarAttribute
			);
		...
		static string CreateDefaultEventNamespace(
			string? aggregateNamespace,
		...
		)
		{
			...
			if (
				...
				|| collectionEventAttribute.ConstructorArguments[0].Value
					is not string rawPropertyName
			...
			if (
				!TypeHelpers.TryGetCollectionDetails(
			...
			)
			{
				...
				TypeHelpers.TryGetIEnumerableElementType(
					parameterType,
					out var enumerableElementType
				)
			)
			{
