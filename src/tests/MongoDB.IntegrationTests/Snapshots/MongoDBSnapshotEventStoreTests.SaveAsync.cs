...
partial class MongoDBSnapshotEventStoreTests
{
	...
	)
	{
		...
		// Act
		bool result = await mongoDbEventStore.SaveAsync(
			aggregate,
			cancellationToken: cancellationToken
		);
		...
		await Assert
			.That(aggregateFromMongoDB.Details.SavedVersion)
			.IsEqualTo(aggregate.Details.SavedVersion);
		await Assert
			.That(aggregateFromMongoDB.Details.CurrentVersion)
			.IsEqualTo(aggregate.Details.CurrentVersion);
		await Assert
			.That(aggregateFromMongoDB.Details.SnapshotVersion)
